...
public class CarListingController {
    ...
    // =========================
    @Operation(
            summary = "Get all pending cars",
            description = "Retrieve a list of all car listings awaiting approval.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "List of pending cars",
                            content = @Content(schema = @Schema(implementation = CarListingResponseDto.class))
                    )
            }
    )
    ...
    public ResponseEntity<List<CarListingResponseDto>> getAllPendingCars() {
        return ResponseEntity.ok(carListingService.getAllPendingCars());
    }

    // =========================
    // Get pending car by ID
    // =========================
    @Operation(
            summary = "Get pending car by ID",
            description = "Fetch details of a pending car listing by ID.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Pending car found"),
                    @ApiResponse(responseCode = "404", description = "Car not found or not pending")
            }
    )
    @GetMapping("/pending/{id}")
    public ResponseEntity<CarListingResponseDto> getPendingCarById(@PathVariable Long id) {
        return ResponseEntity.ok(carListingService.getPendingCarById(id));
    }
    ...
    @Operation(
            ...
            description = "Marks a pending car listing as approved. ADMIN role required.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Car approved successfully"),
                    @ApiResponse(responseCode = "404", description = "Car not found")
            }
    )
    @PreAuthorize("hasRole('ADMIN')")
    @PatchMapping("/{id}/approve")
    public ResponseEntity<Void> approveCar(@PathVariable Long id) {
    ...
    // =========================
    @Operation(
            summary = "Reject a car listing",
            description = "Rejects a pending car listing. ADMIN role required.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Car rejected successfully"),
                    @ApiResponse(responseCode = "404", description = "Car not found")
            }
    )
    @PreAuthorize("hasRole('ADMIN')")
    @PatchMapping("/{id}/reject")
    public ResponseEntity<Void> rejectCar(@PathVariable Long id) {
